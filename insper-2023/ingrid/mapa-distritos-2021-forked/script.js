...
// carrega os dados
Promise.all([
  fetch("dados_distritos_2021.json").then((response) => response.json()),
  fetch("contorno-sp.json").then((response) => response.json())
]).then((files) => {
  // dimensões do mapa
  //const w = 600;
  //const h = 600;
  ...
  function showInfo(e, element_data) {
    ...
    if (ndf === 0) texto = "não possui favelas.";
      ...
      texto = `possui <strong>${format(
        ndf
      )}</strong> favelas de um total de <strong>${format(
        ndp
      )}</strong> domicílios, o que representa um percentual de <strong>${format(
        pct
      )}%</strong>.`;
    }
    ...
    distritos.classed(
      "highlight",
      (d) => d.properties.regiao === district_name
    );
